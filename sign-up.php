...
<link href='https://fonts.googleapis.com/css?family=Roboto:400,300,300italic,500,700,900' rel='stylesheet' type='text/css'>
 <script>
  function useremailAvailability() {
    ...
    var status = document.getElementById('user-emailavailability-status');
    var gmailRegex = /^[a-zA-Z0-9._%+-]+@gmail\.com$/;

    if (gmailRegex.test(emailInput)) {
        status.style.color = "green";
        status.textContent = "Valid Gmail address.";
    } else {
...
<div class="col-md-6 col-sm-6">
  ...
  <div style="display: flex; align-items: center;">
    <select name="countrycode" id="countrycode" style="border: 1px solid #ccc; padding: 5px 10px; 
                background-color: #f7f7f7; border-radius: px; display: flex; 
...
<option value="+81">+81 (Japan)</option>
    ...
    <input type="text" name="contactnumber" id="contactnumber" onBlur="usercontactnoAvailability()"  
           placeholder="e.g. 1234567890" autocomplete="off" pattern="[0-9]{10}" 
           title="only phone number allowed" maxlength="10" required 
